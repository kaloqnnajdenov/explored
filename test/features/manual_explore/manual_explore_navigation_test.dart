...
class FakeVisitedGridRepository implements VisitedGridRepository {
  ...
  @override
  Future<double> fetchExploredAreaKm2({DateTime? start, DateTime? end}) async =>
      0;
...
class FakeManualExploreRepository implements ManualExploreRepository {
  ...
  @override
  List<LatLng> cellBoundary({required String cellId, required double zoom}) =>
      const [LatLng(0, 0), LatLng(0, 1), LatLng(1, 1), LatLng(1, 0)];
    ...
    final router = GoRouter(
      routes: [
        ...
        GoRoute(
          path: '/manual-explore',
          builder: (_, __) =>
              ManualExploreView(viewModel: manualExploreViewModel),
        ),
      ],
    );
